...
async def taixiu(interaction: discord.Interaction, sodiem_cuoc: int, luachon: app_commands.Choice[str]):
    ...
    if current_pts < sodiem_cuoc:
        await interaction.response.send_message(f"❌ Bạn không đủ điểm để cược! Điểm hiện tại của bạn là `{current_pts}`.", ephemeral=True)
        return
    ...
    embed = discord.Embed(
        title="🎲 TRÒ CHƠI TÀI XỈU 🎲",
        description=f"🎲 Xúc xắc: **[{dice1}] [{dice2}] [{dice3}]** ➔ Tổng: **{total}** ({'TÀI' if result == 'tai' else 'XỈU'})"
    )

    if luachon.value == result:
        new_pts = add_points(user_id, sodiem_cuoc)
        embed.color = discord.Color.green()
        embed.add_field(name="🏆 KẾT QUẢ", value=f"🎉 **THẮNG RỒI!** Bạn nhận được **+{sodiem_cuoc} điểm**!\n📈 Điểm mới: `{new_pts}`", inline=False)
    else:
        new_pts = add_points(user_id, -sodiem_cuoc)
        embed.color = discord.Color.red()
        embed.add_field(name="🏆 KẾT QUẢ", value=f"💀 **THUA RỒI!** Bạn mất **-{sodiem_cuoc} điểm**!\n📉 Điểm còn lại: `{new_pts}`", inline=False)

    await interaction.response.send_message(embed=embed)

@bot.tree.command(name="bangxephang", description="Xem Bảng xếp hạng Top tích điểm")
async def bangxephang(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title="🏆 BẢNG XẾP HẠNG TÍCH ĐIỂM TUẦN 🏆",
        color=discord.Color.gold(),
    ...
    embed.description = desc if desc else "Chưa có dữ liệu tích điểm."
    await interaction.response.send_message(embed=embed)

@bot.tree.command(name="diem", description="Kiểm tra điểm tuần và tổng điểm của bạn")
async def diem(interaction: discord.Interaction, user: discord.User = None):
    ...
    embed = discord.Embed(
        title=f"📊 THÔNG TIN ĐIỂM CỦA {target.name.upper()}",
        color=discord.Color.blue()
    )
    embed.add_field(name="📅 Điểm Tuần Hiện Tại", value=f"`{u_info.get('weekly', 0)}` điểm", inline=True)
    embed.add_field(name="🏆 Tổng Điểm Tích Lũy", value=f"`{u_info.get('total', 0)}` điểm", inline=True)
    
    titles_list = u_info.get("titles", [])
    t_str = "\n".join([f"• **[{t}]**" for t in titles_list]) if titles_list else "Chưa có"
    embed.add_field(name="🏅 Danh Hiệu Đã Sở Hữu", value=t_str, inline=False)

    await interaction.response.send_message(embed=embed)

@bot.tree.command(name="setup_game_channel", description="Cài đặt kênh chính để phát Mini-game và Bảng xếp hạng")
...
async def setup_game_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    save_config(cfg)
    await interaction.response.send_message(f"✅ Đã thiết lập thành công {channel.mention} làm kênh Game chính!")

# ==============================================================================
# --- 9. KÍCH HOẠT VÀ CHẠY BOT ---
# ==============================================================================

if __name__ == "__main__":
    keep_alive()  # Chạy server Flask ẩn để Replit/UptimeRobot ping giữ 24/7
    TOKEN = os.getenv("DISCORD_TOKEN")  # Hoặc thay bằng "TOKEN_CỦA_BẠN"
    if TOKEN:
        bot.run(TOKEN)
    else:
        print("[ERROR] Chưa cấu hình DISCORD_TOKEN trong Environment Variables!")
